namespace P13.FamilyTree
{
    ...
    class StartUp
    {
        static void Main()
        {
            ...
            var person = persons.First(p => p.Name == personArgument || p.Birthdate == personArgument);
        ...
        private static void AddRelativesToPerson(List<string> relations, List<Person> persons, Person person)
        {
            ...
            {
                if (line.Contains(person.Name) == false && line.Contains(person.Birthdate) == false)
                {
                    continue;
                }
                ...
                var parentArgument = lineTokens[0];
                var childArgument = lineTokens[1];
